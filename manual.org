...
- =STREAM= :: (Boolean) Stream responses when using this backend, if
  supported.
- =REQUEST-PARAMS= :: A plist of additional HTTP request parameters
  (as plist keys) and their values supported by the API.  Its contents
  are API-specific, and can be used to set parameters that gptel does
  not provide user options for.
