...
| =gptel-org-archive-done-state=               | =nil=   | TODO state to set after summary (e.g. "ARCHIVED") |

** Auto State Transition After Summary

When =gptel-org-archive-done-state= is non-nil (e.g. ="ARCHIVED"=):
- After =gptel-org-prepare-archive= successfully generates a summary,
  the task's TODO state is automatically transitioned to this value
- Applies to both regular DONE tasks and persistent tasks
- The state must be defined in the buffer's =#+SEQ_TODO= keywords
- Example: =AI-DONE= → (summary generated) → =ARCHIVED=
- Implemented via =gptel-org-archive--maybe-set-done-state=
- When nil (default), no state transition occurs (backward compatible)
...
** State Transition
- =gptel-org-archive--maybe-set-done-state= (pos) → transitions task at POS to =gptel-org-archive-done-state= if configured; no-op when nil
