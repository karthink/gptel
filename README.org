...
If you want to act on rendered reasoning or tool blocks, use
=gptel-post-reasoning-insert-functions= or
=gptel-post-tool-insert-functions=.  These are UI hooks: they run when
gptel inserts these blocks into a buffer, such as via =gptel-send=,
and are not part of the =gptel-request= API.
...
| =gptel-post-tool-insert-functions=      | Runs after gptel inserts a tool result block into a buffer       |
| =gptel-post-reasoning-insert-functions= | Runs after gptel inserts and folds a reasoning block in a buffer |
