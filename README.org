...
**** How to open the chat buffer in current window?
#+html: </summary>
By default, gptel opens a new buffer for chat UI according to =display-buffer-alist= setting. This can be modified to use the current window with following:  

#+begin_src emacs-lisp
(setopt gptel-display-buffer-action '(pop-to-buffer-same-window))
#+end_src
#+html: </details>
#+html: <details><summary>
