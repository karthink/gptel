...
With =gptel-mcp-connect= you can register pragmatically MCP servers and selectively activate MCP tools. It also auto-starts the MCP servers is those are not already started.

#+begin_src emacs-lisp
  (gptel-mcp-connect '(("fetch") ("youtube-opt") ("filesystem" "list_directory" "get_file_info")))
#+end_src
