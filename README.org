...
By default, files in a version control system that are not project files will not be added to the context. To be able to add these files, set =gptel-context-exclude-git-ignored= to =nil=. Note that remote files are always included, regardless of the value of =gptel-context-exclude-git-ignored=.
