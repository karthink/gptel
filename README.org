...
~AWS_ACCESS_KEY_ID~, ~AWS_SECRET_ACCESS_KEY~, ~AWS_SESSION_TOKEN~ (optional), or if present, can use ~AWS_PROFILE~ to get these directly from the ~aws~ cli. You can also make use of [[https://docs.aws.amazon.com/bedrock/latest/userguide/api-keys.html][Bedrock API keys]] by setting the =:bedrock-api-key= parameter:
