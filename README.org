...
- Supports introspection, so you can see exactly what will be sent.  You can inspect and modify queries before sending them.
...
** Contents :toc:
  ...
  - [[#faq][FAQ]]
      ...
      - [[#adding-models-to-gptel-backends][Adding models to gptel backends]]
...
- Obtain a reference to the backend.  For ChatGPT, this is just
  `gptel-backend`. For other backends, this should be the return value of the
  (gptel-make-<backend-name>) function.
