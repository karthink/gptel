...
2. Direct credentials from the ~AWS_ACCESS_KEY_ID~ and ~AWS_SECRET_ACCESS_KEY~ environment variables. The ~AWS_SESSION_TOKEN~ variable is also used if available.
3. An AWS profile from the ~AWS_PROFILE~ environment variable.
