...
`gptel-make-tool', which see."
  function name
  description args async category include)
...
(cl-defun gptel-make-tool (&key name function description args
                             async category include)
...
continue the request."
  (let ((tool (gptel--make-tool
               ...
               :args args :async async :category category
               :include include))
        (category (or category "misc")))
    (setf (alist-get
           name (alist-get category gptel--known-tools
                           nil nil #'equal)
           nil nil #'equal)
          tool)))
    ...
    (let* ((pb (lambda (s) (propertize s 'face 'font-lock-builtin-face)))
           ...
           (entries-info
            (cl-loop
             ...
             unless (memq key '(:data :stream :history))
             collect
           ...
           (entries-data
            (cl-loop
             ...
             unless (memq key '(:messages :contents :query))
             collect
             (list idx `[,(funcall pb (symbol-name key))
                         ,(funcall ps (funcall fmt val))]))))
