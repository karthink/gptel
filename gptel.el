...
(defvar gptel-tools
  (list (gptel-make-tool
         :function (lambda (_loc unit) (if (equal unit "farenheit") "24 farenheit" "-3 celsius"))
         :name "get_weather"
         ...
         :args (list '(:name "location" :type "string"
                     ...
                     '(:name "unit" :type "string" :enum ("celsius" "farenheit")
                       ...
                       :optional t)))
        (gptel-make-tool
         :function (lambda () "Anchorage, AK")
         :name "get_location"
         :description "Get the user's location.  No inputs are needed.  Returns a city name and state.")))
...
(cl-defgeneric gptel--inject-prompt (_backend data new &optional position)
  "Append NEW to existing prompts in query DATA.
         ...
         (letrec ((name (plist-get tool-call :name))
           ...
           (when-let* ((tool-spec
             ...
             (if (gptel-tool-async tool-spec)
               ...
               (let ((result (apply (gptel-tool-function tool-spec)
                                    arg-values)))
