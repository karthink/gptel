...
See gptel's customization options for all available settings."
  (declare (indent 1))
  (setf (alist-get name gptel--known-presets nil nil #'equal)
        keys))
...
kill-ring."
  (interactive
   (list (intern (completing-read "Save to (existing or new) preset: "
                                  gptel--known-presets))
...
(defun gptel--handle-augment (fsm)
  ...
  (let* ((info (gptel-fsm-info fsm)))
    ...
    (if gptel-augment-handler-functions
        (let ((augment-total
               (if (local-variable-p 'gptel-augment-handler-functions)
                   (+ (length (remq t gptel-augment-handler-functions))
                      (length (default-value 'gptel-augment-handler-functions)))
                 (length gptel-augment-handler-functions)))
              (augment-idx 0))
          (with-current-buffer (plist-get info :buffer)
