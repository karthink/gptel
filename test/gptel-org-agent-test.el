...
(ert-deftest gptel-org-agent-test-task-ib-has-ref-level ()
  "After TodoWrite creates a task IB, it has gptel-org--ref-level set.
This ensures auto-approved tool results land at task-level+1 (child of
task), not at task-level (sibling), by giving
`gptel--tool-heading-ref-level' step 1 a valid value for task IBs.

The agent heading is at level 2, so the task heading should be at
level 3 and the task IB's `gptel-org--ref-level' should be 4 (= task
heading level + 1)."
  (let ((org-inhibit-startup t)
        (org-todo-keywords '((sequence "AI-DO" "AI-DOING" "|" "AI-DONE")))
        (gptel-org-todo-keywords '("AI-DO" "AI-DOING"))
    ...
    (with-temp-buffer
      (delay-mode-hooks (org-mode))
      (insert "** AI-DOING My task :main@agent:\n")
      (goto-char (point-min))
      ;; Set up FSM like the redirect tests do, so write-todo-org's
      ;; redirect-markers-to-heading branch also runs.
      (goto-char (point-max))
      (let* ((pos-marker (point-marker))
        ...
        ;; Both tasks got task IBs created by create-todo-heading.
        ;; Each one must carry gptel-org--ref-level = 1 + task-heading-level.
        ;; Agent is level 2, tasks are level 3 → ref-level should be 4.
        (dolist (pair gptel-org-agent--todo-task-ibs)
          (let* ((content (car pair))
                 (task-ib (cdr pair)))
            (should (buffer-live-p task-ib))
            (with-current-buffer task-ib
              (save-excursion
                ...
                (let ((task-level (org-current-level)))
                  ...
                  (should (= (1+ task-level) gptel-org--ref-level))
                  (should (= 4 gptel-org--ref-level))
                  (gptel-org--debug
                   "task-ib-has-ref-level: content=%S task-level=%d ref-level=%d"
                   content task-level gptel-org--ref-level))))))
        ;; Clean up the task IBs so we don't leak them past the test.
        (dolist (pair gptel-org-agent--todo-task-ibs)
