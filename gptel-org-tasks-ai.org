...
When =gptel-org-use-todo-keywords= is enabled, the state machine supports an iterative FEEDBACK cycle:

#+begin_example
AI-DO ──gptel-send──→ AI-DOING ──response complete──→ FEEDBACK ──gptel-send──→ AI-DOING → ...
  │                      │                                                        │
  │                      └──gptel-abort──→ CANCELED                               └──→ FEEDBACK → ... → DONE
  │
  └──(manual)──→ FEEDBACK (user review)
#+end_example

Without keyword mode, the simpler legacy flow applies:

#+begin_example
AI-DO ──gptel-send──→ AI-DOING ──response complete──→ (cleared/AI-DONE)
                         │
...
#+begin_example
#+SEQ_TODO: TODO(t) AI-DO(a) AI-DOING(i) DOING FEEDBACK(f) | AI-DONE(d) DONE(D) CANCELED(c)
#+end_example

- =AI-DO= — task ready for AI processing
- =AI-DOING= — task being processed (auto-transition on =gptel-send=)
- =FEEDBACK= — AI finished, awaiting user review/input (auto-transition from AI-DOING on response completion in keyword mode)
- =AI-DONE= — completed by AI
- =DONE= — completed (user ends the FEEDBACK cycle)
...
| AI-DOING → FEEDBACK  | Response complete | =gptel-org-use-todo-keywords= enabled             |
| AI-DOING → (cleared) | Response complete | Legacy mode (keyword mode disabled)               |
| FEEDBACK → AI-DOING  | =gptel-send=      | User sends from FEEDBACK heading (keyword mode)   |
| AI-DOING → CANCELED  | =gptel-abort=     | Active task marker exists                         |
| Any → DONE           | Manual            | User ends the feedback cycle                      |
